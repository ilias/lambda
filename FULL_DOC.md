...
High‑performance lambda calculus interpreter in C# featuring:

- Lazy & eager evaluation (CEK machine)
- Pratt‑parsed user infix operators (custom precedence / associativity)
- Advanced macro system (multi‑clause, guards, structural & variadic patterns, wildcards, numeric literal patterns)
- Optional native arithmetic & list fast paths (`:native on|off`)
- Structural equivalence helpers (`alphaEq`, `betaEq`, `hashEq`, `etaEq`)
- Rich standard library (~200 combinators & utilities)
- CLI, Web API, and streaming Web UI (SSE / WebSocket)

## Documentation

| Topic | File |
|-------|------|
| Language syntax, grammar, macros, operators | LANGUAGE.md |
| REPL / colon commands, tracing, stats | COMMANDS.md |
| Build, packaging, embedding, Docker | COMPILER.md |
| Theory / background | THEORY.md |
| Standard library source | stdlib.lambda |

Extras: `tests.lambda` (examples/specs), `exit.lambda` helper. If Pandoc is installed the build script can emit `readme.html` for the Web UI.
...
Open http://localhost:5000 and evaluate `succ 41`.
...
|---------|------|---------|
| Core library | src/ | Interpreter engine (parser, CEK, macros, natives) |
| CLI REPL | src-cli/ | Interactive shell & file runner |
| Web API | src-web/ | HTTP evaluation & load endpoints |
| Web UI | src-webui/ | Browser REPL + streaming logs & filters |

See `COMPILER.md` for build, packaging, Docker & embedding details.

## Contributing

Read the reference docs before PRs. Implement first, document after. Use `tests.lambda` with `alphaEq` for structural assertions.
...
Continue in `LANGUAGE.md` for the full language reference.
